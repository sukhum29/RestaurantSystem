...
class CreateOrders(tk.Toplevel):
    def __init__(self, parent, func):
        ...
        self.fc_name = ttk.Label(self.cst_lbl, text=f"\"{self.fac_info[0]}\"", font="Helvetica 14 bold")
        ...
        self.send_to_ch = ttk.Button(self.main_frame,text='Send to kitchen', state=tk.DISABLED, command=self.send_to_kitchen)
    ...
    def add_records(self, orders):
        try:
          ...
          for order in orders:
            ...
            self.fac_db.insert_spec_config(spec_insert_query, (order_id, table_num, order_name, order_quantity, order_status))
        except Error as e:
          print(e)
            ...
            if table_num:
                for order in self.order_ls:
                    if order.retrieve_data()[0] == 'Select a meal':
                        messagebox.showerror("Meal is not selected", "Please, either fill \"Select a meal\" or delete that row to continue!")
                        return False
                    else:
                        orders.append((order.retrieve_data(), table_num))
                self.add_records(orders)
                usr_resp = messagebox.askyesno("Success", "Order have been sent to the kitchen, you can access it through the main window, do you wish to get more orders?")
                if usr_resp:
                    self.clear()
                else:
                    self.destroy()
            else:
                messagebox.showerror("Empty Fields", "Please enter a valid table number!")
        except Error as e:
            print(e)
