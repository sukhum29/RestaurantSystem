...
class ConfigWindow(tk.Toplevel):
    ...
    def validate_product(self, price, name):
        if ( self.is_float(price) and float(price) <= 10000000) and (len(name) <= 20):
            ...
            usr_resp = messagebox.showerror("Wrong inputs", "The price you have entered exceeds maximum allowed (10 million Forints) and product name should be less than 20 characters long")
            self.food_price_entry.delete(0, tk.END)
            self.food_name_entry.delete(0, tk.END)
            return False
        else:
            messagebox.showerror("Wrong input", "Please enter the product name(max. 20 characters long) and price(max 10 mln forints) correctly!")
            return False
